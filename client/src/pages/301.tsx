...
function ConfigWindow(props: any) {
    const { study, setStudy, config, setConfig, configWindow, setConfigWindow } = props;

    
    const antonym = (newWord: boolean) => (e:any) => {
        setConfigWindow('');
        if (newWord) {
            axios.get('/api/antonym_word').then(({ data }) => {
                config.src = '/media/word/' + data[0].words[0].word + '.mp3';
                config.corrected = new Array(data.length);
                config.interval = config.defaultInterval;
                setStudy({ toggle: '반의어 찾기', '반의어 찾기': {data, config}})
            });
        } else {
            const data = study[configWindow].data;
            config.src = '/media/word/' + data[0].words[0].word + '.mp3';
            config.corrected = new Array(data.length);
            config.interval = config.defaultInterval;
            setStudy({ toggle: '반의어 찾기', '반의어 찾기': { data, config } });
    ...
    return (
        <Popup onExit={(e) => { setConfigWindow('') }}>
            <div className="config">
                ...
                <div>
                    <button disabled={configWindow in study ? false : true} onClick={antonym(false)}>기존 단어</button>
                    <button onClick={antonym(true)}>새로운 단어</button>
                </div>
            </div> 
        </Popup>
    )
...
export default function word301(props: any) {
    ...
    return (
        <>
            ...
            <button onClick={showPopup('반의어 찾기')}>반의어 찾기</button>
