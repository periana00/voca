...
(async () => {
    console.log(await execute('select count(*) from words where chapter between 1 and ""'));
})();
...
router.get('/api/words', async (req, res) => {
    ...
    const words = await getWords(params);
    console.log(params);
    res.cookie('sort', '0', {path: '/1'});
    res.cookie('reset', '0', {path: '/1'});
    res.send(words);
})


router.get('/', (req, res) => {
    res.cookie('path', '1', {path: '/1'});
